...
from losses.semantic_pos_loss import SemanticPosLoss


class SpixelNet(nn.Module):
    expansion = 1

    def __init__(self, img_size, num_channels, num_frames, batch_norm = True):
        ...
        self.conv0a = conv(self.batchNorm, self.num_channels * self.num_frames, 16, kernel_size=3)
        ...
        self.criterion = SemanticPosLoss()
    ...
    def forward(self, x, y):
        ...
        curr_spixel_map = update_superpixel_map(self.initial_spixelmap_index, prob0)
        curr_spixel_map = post_process_superpixels(curr_spixel_map)

        loss = self.criterion(prob0, y, self.XY_features)
...
if __name__ == "__main__":
    model = SpixelNet(num_channels=4, num_frames=1, img_size=208).cuda()
